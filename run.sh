...
#cluster_cmd="qsub -terse -l mfree=${mem_arg}M -l h_rt=48:0:0 -l 'hostname=!s022&!s025' -o $out_path -e $err_path -pe serial {threads}"

snakemake \
    --cluster "$cluster_cmd" \
    --cluster-cancel "qdel" \
    -j 128 \
    $* --cores 50 --resources mem_mb=1500000 --set-resource-scopes mem_mb=global threads=global \
