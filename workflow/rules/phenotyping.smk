...
rule run_cellprofiler:
    ...
    #retries: 2
    shell:
        'cellprofiler -c -r -p {input.pipeline} -i ' + phenotyping_dir + '{wildcards.prefix}/cellprofiler -o ' + phenotyping_dir + '{wildcards.prefix}/cellprofiler/{wildcards.pipeline} || test $? = 0 -o $? = 1 && echo \'""\' > {output.cell_file}'
