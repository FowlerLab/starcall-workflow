...
rule make_qc_read_plots:
    ...
    run:
        ...
        for i,let in enumerate('GTAC'):
            if has_library: axes[0,0].plot(np.mean(barcode_mat == let, axis=0), '--C' + str(i))
        ...
        if 'blainey' in wildcards.prefix:
            axes[0,0].set_title('Image set 1: Feldman method')
        else:
            axes[0,0].set_title('Image set 1: STARCall')
        ...
        if 'blainey' in wildcards.prefix:
            axes[0,1].set_title('Image set 2: Feldman method')
        else:
            axes[0,1].set_title('Image set 2: STARCall')
